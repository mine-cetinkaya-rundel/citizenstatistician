...
Citizen Statistician is back from a hiautus! I hope to post more regularly in 
...
it shortly. But first, here is a picture of my 2.5 year old "dealing" with 
time zone change. 
...
zone is Eastern Daylight Time, which is GMT-4, incidated by the `-04:00` at the 
...
The Darsky API mentions UNIX time (seconds since midnight GMT on 1 Jan 1970), so let's convert it.
...
Here I have split the day into four time periods with `cut()`, calculated mean 
...
need to retrain my brain to process Celcius information quickly. I believe 
this is one of the accepted uses of dual axes though, since the same data is 
being plotted on different scales (degrees Fahrenheit vs. Celcius).
...
Weather has been lovely in Edinburgh for the last few days. Apparently it's 
called "taps aff" weather^[https://www.edinburghnews.scotsman.com/news/weather/edinburgh-weather-this-is-when-soaring-temperatures-will-hit-the-capital-1-4925959], 
