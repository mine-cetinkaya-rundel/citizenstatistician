...
```{r include = FALSE}
replies_raw <- read_rds(here::here("content/post/2021-02-28-themoment-tweets/data/replies.rds"))
```
...
march_13_words %>%
  ...
  with(wordcloud(word, n, max.words = 30, colors = viridis::viridis(n = 30)))
```
...
replies %>%
  transmute(
    ...
    tom_hanks    = str_detect(text, "^tom hanks| tom hanks"),
    nba          = str_detect(text, "^nba| nba"),
    spring_break = str_detect(text, "^spring break| spring break")
    ) %>%
  summarise(
    n_tom_hanks    = sum(tom_hanks),
    p_tom_hanks    = n_tom_hanks / nrow(replies),
    n_nba          = sum(nba),
    p_nba          = n_nba / nrow(replies),
    n_spring_break = sum(spring_break),
    p_spring_break = n_spring_break / nrow(replies)
  )
```
