...
```{r include = FALSE}
replies_raw <- read_rds(here::here("content/post/2021-03-01-themoment-tweets/data/replies.rds"))
```
...
```{r out.width = "100%", fig.asp = 0.4, fig.alt = "Very few tweets mentioning dates December through March, then a steady increase until a peak on March 13, and then a decline with a tail extending all the way to the end of May. There were over 200 tweets mentioning March 13,"}
...
replies %>%
  ...
  summarise(across(tom_hanks:spring_break, sum)) %>%
  mutate(across(tom_hanks:spring_break, ~ . / nrow(replies), .names = "p_{.col}"))
```

Only about 1% of tweets for Tom Hanks and roughly 3% of tweets for NBA and spring break. Not too many actually, but still more than I expected for especially Tom Hanks.
...
There are certainly some holes in this analysis. My regular expressions could be more precise and relying on `readr::parse_number()` solely to get dates is likely not bullet proof. 
